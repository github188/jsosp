app.controller('HttpCheckCtrl', ['$scope','$state','$stateParams','$http','$filter',function($scope,$state,$stateParams,$http,$filter){
  ...
  $scope.telephone="";
  $scope.platformname="";
  $http.post("auto/httpCheck/list.do?pageSize="+$scope.pageSize+"&pageIndex=0").success(function(response) {
  ...
  $scope.changepage = function(index){
    ...
    if($scope.httpname!=undefined){
	  ...
	  $http.post("auto/httpCheck/list.do?pageSize="+$scope.pageSize+"&pageIndex="+$scope.pages[$scope.currentpage].index+"&httpService="+httpService).success(function(response) {
		  var resp =angular.fromJson(angular.fromJson(response));
		  console.log(resp);
		  var returnCode = resp.returnCode;
		  var message = resp.message;

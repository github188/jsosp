app.controller('RdsSlowLogCtrl', ['$scope','$state','$stateParams','$http','$filter',function($scope,$state,$stateParams,$http,$filter){
  ...
  $http.post("auto/rdsSlowLog/list.do?pageSize="+$scope.pageSize+"&pageIndex=0").success(function(response) {
  ...
  $scope.changepage = function(index){
    ...
    if($scope.rdsname!=undefined){
	  ...
	  $http.post("auto/rdsSlowLog/list.do?pageSize="+$scope.pageSize+"&pageIndex="+$scope.pages[$scope.currentpage].index+"&rdsHosts="+rdsHosts).success(function(response) {
		  var resp =angular.fromJson(angular.fromJson(response));
		  console.log(resp);
		  var returnCode = resp.returnCode;
		  var message = resp.message;

app.controller('LockwaitCtrl', ['$scope','$state','$stateParams','$http','$filter',function($scope,$state,$stateParams,$http,$filter){
  ...
  $scope.query=function(){
	  $scope.log="正在查询";
	  $http.post("auto/lockwait/lockwait.do?platform="+$scope.database.code).success(function(response) {
		  var resp =angular.fromJson(angular.fromJson(response));
		  console.log(resp);
		  var returnCode = resp.returnCode;
		  var message = resp.message;
		  ...
		  $scope.lockwaits = resp.result;
		  $scope.counts = $scope.lockwaits.length;
		  if($scope.lockwaits.length==0){
			  $scope.log="此库没有锁等待";
		  }else{
			  $scope.log="查询完毕";
		  }

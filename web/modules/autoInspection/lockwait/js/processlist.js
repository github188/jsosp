app.controller('ProcesslistCtrl', ['$scope','$state','$stateParams','$http','$filter',function($scope,$state,$stateParams,$http,$filter){
  ...
  $scope.query=function(){
	  $scope.log="正在查询";
	  $http.post("auto/lockwait/processlist.do?platform="+$scope.database.code).success(function(response) {
		  var resp =angular.fromJson(angular.fromJson(response));
		  console.log(resp);
		  var returnCode = resp.returnCode;
		  var message = resp.message;
		  ...
		  $scope.processlists = resp.result;
		  $scope.counts = $scope.processlists.length;
		  if($scope.processlists.length==0){
			  $scope.log="无数据库活动任务";
		  }else{
			  $scope.log="查询完毕";
		  }

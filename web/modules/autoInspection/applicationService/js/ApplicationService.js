app.controller('ApplicationServiceCtrl', ['$scope','$state','$stateParams','$http','$filter',function($scope,$state,$stateParams,$http,$filter){
  ...
  $scope.userName="";
  $scope.telephone="";
  $http.post("auto/btmanage/list.do").success(function(response) {
			  ...
			  if(returnCode==0){
				  ...
				  $scope.bts1 = resp.result.slice(0,resp.result.length/2);
				  $scope.bts2 = resp.result.slice(resp.result.length/2,resp.result.length);
			  }else{
				  alert('查询错误');
			  }
		  });

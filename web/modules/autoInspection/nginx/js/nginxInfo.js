app.controller('NginxInfoCtrl', ['$scope','$state','$stateParams','$http','$filter',function($scope,$state,$stateParams,$http,$filter){
  ...
  $scope.sourceip="";
  $http.post("auto/nginx/infoquery.do?pageSize="+$scope.pageSize+"&pageIndex=0"+"&sourceip="+$scope.sourceip
      ...
      for (var i = $scope.nginxs1.length - 1; i >= 0; i--) {
    	  ...
    	  data.name=$scope.nginxs1[i].host_name;
  ...
  $scope.changepage = function(index){
    ...
    var starttime=$filter('date')($scope.dt,'yyyy-MM-dd HH:mm:ss');
    var endtime=$filter('date')($scope.dt1,'yyyy-MM-dd HH:mm:ss');
	  $http.post("auto/nginx/infoquery.do?pageSize="+$scope.pageSize+"&pageIndex="+$scope.pages[$scope.currentpage].index+"&sourceip="+$scope.sourceip
			  +"&starttime="+starttime+"&endtime="+endtime).success(function(response) {
		  var resp =angular.fromJson(angular.fromJson(response));
		  console.log(resp);
		  var returnCode = resp.returnCode;

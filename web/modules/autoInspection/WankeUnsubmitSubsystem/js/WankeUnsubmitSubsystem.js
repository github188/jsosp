app.controller('WankeUnsubmitSubsystemCtrl', ['$scope','$state','$stateParams','$http','$filter',function($scope,$state,$stateParams,$http,$filter){
  ...
  $scope.wankes = [
      {id:'1',check_time:'运维中',check_date:'John',subsystems:'2016-5-12',out_records:'@jh',request_time:'a@company.com',msg_code:'上海'},
      {id:'2',check_time:'运维中',check_date:'Bill',subsystems:'2016-5-9',out_records:'@bg',request_time:'bg@company.com',msg_code:'北京'},
      {id:'3',check_time:'暂停',check_date:'Bobo',subsystems:'2015-5-12',out_records:'@hz',request_time:'c@company.com',msg_code:'武汉'},
      {id:'4',check_time:'暂停',check_date:'Bobo',subsystems:'2015-5-12',out_records:'@hz',request_time:'c@company.com',msg_code:'武汉'},
      {id:'5',check_time:'暂停',check_date:'Bobo',subsystems:'2015-5-12',out_records:'@hz',request_time:'c@company.com',msg_code:'武汉'},
      {id:'6',check_time:'实施中',check_date:'Lily',subsystems:'2013-5-12',out_records:'@li',request_time:'dd@company.com',msg_code:'上海'}
    ];
  $scope.counts=0;
  $scope.pageSize=6;
  $scope.currentpage=0;
  ...
  $http.post("auto/wankesusubmit/list.do?pageSize="+$scope.pageSize+"&pageIndex=0").success(function(response) {
  ...
  $scope.changepage = function(index){
    ...
    if ($scope.pages[index].index==$scope.allpages) {$scope.rightpagestatus=$scope.status1;};
	  $http.post("auto/wankesusubmit/list.do?pageSize="+$scope.pageSize+"&pageIndex="+$scope.pages[$scope.currentpage].index).success(function(response) {
		  var resp =angular.fromJson(angular.fromJson(response));
		  console.log(resp);
		  var returnCode = resp.returnCode;
		  var message = resp.message;
  ...
  $scope.tableblur = function (index){
	  	console.log(index);
	  	for(var i=0;i<$scope.wankes.length;i++){
	  		if(i==index){
	  			$scope.wankes[i].style="background-color:#0c67c4;";
	  		}else{
	  			$scope.wankes[i].style="";
	  		}
	  	}
	  }

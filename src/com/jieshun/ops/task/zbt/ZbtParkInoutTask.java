...
public class ZbtParkInoutTask extends TimerTask {
	
	@Autowired
	private SMSSender smsSender;
	public ZbtParkInoutTask(String driver, String url, String user, String password, String parkcodes, String phones) {
		super();
		this.driver = driver;
		this.url = url;
		this.user = user;
	...
	private static final Logger logger = Logger.getLogger(ZbtParkInoutTask.class);
...
//			ResultSet rs = statement.executeQuery("select id,name from np_eq_park where status='NORMAL' and code in ('"+parkcodes+"')" );
			...
			Date start = new Date(( System.currentTimeMillis() - (24 * 60 * 60 * 1000))-(60*1000));
			...
			msg.append("周泊通检测,时间段:" + startdate + "---" + enddate);
			while (rs.next()) {
				...
				ResultSet in0 = statement1
						.executeQuery("select count(1) from np_ds_park_in where send_flag='0' and park_id = '" + id
								+ "' and create_time>'" + startdate + "'and create_time<'" + enddate + "'");
				while (in0.next()) {
					parkin0 = in0.getInt(1);
				}
				ResultSet in1 = statement1
						.executeQuery("select count(1) from np_ds_park_in where send_flag='1' and park_id = '" + id
								+ "' and create_time>'" + startdate + "'and create_time<'" + enddate + "'");
				while (in1.next()) {
				...
				ResultSet out0 = statement2
						.executeQuery("select count(1) from np_ds_park_out where send_flag='0' and park_id = '" + id
								+ "' and create_time>'" + startdate + "'and create_time<'" + enddate + "'");
				while (out0.next()) {
					parkout0 = out0.getInt(1);
				}
				ResultSet out1 = statement2
						.executeQuery("select count(1) from np_ds_park_out where send_flag='1' and park_id = '" + id
								+ "' and create_time>'" + startdate + "'and create_time<'" + enddate + "'");
				while (out1.next()) {

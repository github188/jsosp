...
import com.jieshun.ops.autode.dao.HttpCheckDAO;
...
public class HttpCheckImpl implements IHttpCheckService {
	
	@Autowired
	private HttpCheckDAO httpCheckDAO;

	private static final Logger logger = Logger.getLogger(HttpCheckImpl.class.getName());
	...
	public String list(int pageIndex,int pageSize,String httpService) {
		...
		try {
			...
			Map<String, Object> redisBigTableCounts = httpCheckDAO.queryCounts(httpService);
			
			if(pageSize==0){
				pageSize=Integer.parseInt(redisBigTableCounts.get("COUNTS").toString());
			}
			
			List<Map<String, Object>> redisBigTableLists = httpCheckDAO.list(pageIndex*pageSize, pageSize,httpService);
			...
			List<Map<String, Object>> httpLists = httpCheckDAO.listHttp();
